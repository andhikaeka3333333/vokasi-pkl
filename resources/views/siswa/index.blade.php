    ...
    <div x-data="{
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-200">
                            ...
                            <tr class="{{ $loop->even ? 'bg-red-50' : 'bg-white' }} border-b border-gray-50 transition duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-center items-center gap-2">
                                        ...
                                        {{-- Edit --}}
                                        <a href="{{ route('siswa.edit', $siswa->id) }}" class="p-1.5 text-amber-600 hover:bg-amber-100 rounded-md transition">
                ...
                {{-- --- CUSTOM PAGINATION (BUKAN BAWAAN LARAVEL) --- --}}
                @if ($siswas->hasPages())
                <div class="px-6 py-4 bg-white border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    {{-- Info Data --}}
                    <p class="text-sm text-gray-600">
                        Menampilkan <span class="font-bold">{{ $siswas->firstItem() }}</span>
                        sampai <span class="font-bold">{{ $siswas->lastItem() }}</span>
                        dari <span class="font-bold">{{ $siswas->total() }}</span> data
                    </p>
                    ...
                    <nav class="flex items-center gap-1">
                        ...
                        @else
                            <a href="{{ $siswas->previousPageUrl() }}" class="px-3 py-2 bg-white text-gray-600 hover:bg-red-50 hover:text-[#C52222] border border-gray-200 rounded-lg transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                            </a>
                        @endif
                        ...
                        @foreach ($siswas->getUrlRange(max(1, $siswas->currentPage() - 2), min($siswas->lastPage(), $siswas->currentPage() + 2)) as $page => $url)
                            @if ($page == $siswas->currentPage())
                                <span class="px-4 py-2 bg-[#C52222] text-white font-bold rounded-lg border border-[#C52222]">
                                    {{ $page }}
                                </span>
                            @else
                                <a href="{{ $url }}" class="px-4 py-2 bg-white text-gray-600 hover:border-[#C52222] hover:text-[#C52222] border border-gray-200 rounded-lg transition">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
                        ...
                        @if ($siswas->hasMorePages())
                            <a href="{{ $siswas->nextPageUrl() }}" class="px-3 py-2 bg-white text-gray-600 hover:bg-red-50 hover:text-[#C52222] border border-gray-200 rounded-lg transition">
        ...
        {{-- --- MODAL DETAIL --- --}}
        <div x-show="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50" x-transition x-cloak>
            <div @click.away="isModalOpen = false" class="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-gray-100">
                <div class="bg-[#C52222] p-6 text-white flex justify-between items-center">
                    ...
                    <button @click="isModalOpen = false"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                </div>
                ...
                <div class="bg-gray-50 p-4 px-8 flex justify-end">
                    <button @click="isModalOpen = false" class="bg-gray-200 text-gray-700 font-semibold py-2 px-6 rounded-lg hover:bg-gray-300 transition">Tutup</button>
                </div>
            </div>
        </div>

        {{-- --- MODAL KONFIRMASI HAPUS --- --}}
        <div x-show="isDeleteModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black bg-opacity-50" x-transition x-cloak>
            <div @click.away="isDeleteModalOpen = false" class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden p-6 text-center">
                ...
                <p class="text-gray-500 mb-6 text-sm">Yakin mau hapus data <span class="font-bold text-gray-800" x-text="selectedSiswa.nama"></span>?</p>
                <div class="flex justify-center gap-3">
                    <button @click="isDeleteModalOpen = false" class="px-6 py-2.5 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200 transition text-sm">Batal</button>
                    <form :action="deleteRoute" method="POST">
                        @csrf @method('DELETE')
                        <button type="submit" class="px-6 py-2.5 bg-red-600 text-white font-semibold rounded-xl hover:bg-red-700 transition shadow-lg text-sm">Ya, Hapus</button>
                    </form>
                </div>
            </div>
        </div>
